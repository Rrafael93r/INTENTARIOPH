...
import { getImpresorasPos, updateImpresoraPos } from '../../servicios/impresoraPosService';
import FormularioCrearImpresoraPos from '../FormulariosCrear/FormularioCrearImpresoraPos';
import FormularioEditarImpresoraPos from '../FormulariosEditar.tsx/FormularioEditarImpresoraPos';

const TablaImpresorasPos: React.FC = () => {
    ...
    const loadImpresoras = async () => {
        try {
            const data = await getImpresorasPos();
            setImpresoras(data);
        } catch (err) {
            setError('Error al cargar impresoras pos');
            console.error(err);
        } finally {
            setLoading(false);
        }
        ...
        try {
            ...
            if (result.isConfirmed) {
                ...
                if (itemToDisable) {
                    ...
                    await updateImpresoraPos(id, updatedItem);
    ...
    if (loading) return <div className="flex justify-center items-center p-10"><div className="animate-spin rounded-full h-8 w-8 border-b-2 border-orange-500"></div><span className="ml-2 text-gray-600">Cargando impresoras POS...</span></div>;
                        ...
                        <div className="inline-block align-bottom bg-white rounded-xl text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-4xl sm:w-full">
                            <div className="bg-white px-6 py-4 border-b border-gray-100 flex justify-between items-center">
                                <h3 className="text-lg leading-6 font-semibold text-gray-800 flex items-center">
                                    <i className="bi bi-printer mr-2 text-orange-500"></i> Nueva Impresora POS
                                </h3>
                            ...
                            <div className="px-6 py-5 sm:p-6 bg-gray-50">
                                <FormularioCrearImpresoraPos handleClose={() => { handleClose(); loadImpresoras(); }} />
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                    <div>
                        <h2 className="text-xl font-bold text-gray-800 m-0 flex items-center gap-2">
                            <i className="bi bi-printer text-orange-500"></i>
                            Impresoras POS
                        </h2>
                        <p className="text-sm text-gray-500 mt-1 mb-0">Gestión de impresoras de punto de venta</p>
                    </div>
                    ...
                    <tbody>
                        {currentImpresoras.length > 0 ? (
                            currentImpresoras.map((impresora) => (
                                <tr key={impresora.id} className="bg-white border-b border-gray-100 hover:bg-orange-50 transition-colors">
                                    <td className="px-6 py-4 font-medium text-gray-900">{impresora.id}</td>
                                    <td className="px-6 py-4">{typeof impresora.marca === 'object' && impresora.marca !== null ? impresora.marca.nombre : impresora.marca}</td>
                                    <td className="px-6 py-4">{impresora.modelo}</td>
                                    <td className="px-6 py-4 font-mono text-xs">{impresora.serial}</td>
                                    <td className="px-6 py-4">
                                        <span className={`px-2.5 py-1 rounded-full text-xs font-medium ${impresora.estado === 'ACTIVO' ? 'bg-green-100 text-green-800' :
                                            impresora.estado === 'ASIGNADO' ? 'bg-blue-100 text-blue-800' :
                                                impresora.estado === 'DAÑADO' ? 'bg-red-100 text-red-800' :
                                    ...
                                    <td className="px-6 py-4">
                                        {impresora.funcionarios ? (
                                            <div className="flex items-center text-gray-900">
                                                <i className="bi bi-person-badge text-orange-400 mr-2 text-lg"></i>
                                                {`${impresora.funcionarios.nombre} ${impresora.funcionarios.apellido}`}
                                            </div>
                                        ) : (
                            ...
                            <tr>
                                <td colSpan={7} className="px-6 py-8 text-center text-gray-500">
                                    <div className="flex flex-col items-center justify-center">
                                        ...
                                        <p>No se encontraron impresoras POS que coincidan con los filtros.</p>
                                    </div>
                                </td>
                            </tr>
                        )}
                ...
                <div className="fixed inset-0 z-50 overflow-y-auto">
                    <div className="flex items-center justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:p-0">
                        ...
                        <div className="inline-block align-bottom bg-white rounded-xl text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-4xl sm:w-full">
                            <div className="bg-white px-6 py-4 border-b border-gray-100 flex justify-between items-center">
                                <h3 className="text-lg leading-6 font-semibold text-gray-800 flex items-center">
                                    <i className="bi bi-pencil-square mr-2 text-orange-500"></i> Editar Impresora POS
                                </h3>
                ...
                <div className="hidden sm:flex-1 sm:flex sm:items-center sm:justify-between">
                    <div>
                        <p className="text-sm text-gray-700 m-0">
                            Mostrando <span className="font-medium">{currentImpresoras.length}</span> de <span className="font-medium">{filteredImpresoras.length}</span> impresoras POS
                        </p>
                    </div>
                    <div>
...
export default TablaImpresorasPos;
