                        ...
                        {currentPerifericos.length > 0 ? (
                            currentPerifericos.map((periferico) => (
                                <tr key={periferico.id} className="bg-white border-b border-gray-100 hover:bg-orange-50 transition-colors">
                                    ...
                                    <td className="px-6 py-4">
                                        <span className={`px-2.5 py-1 rounded-full text-xs font-medium ${periferico.estado?.nombre?.toUpperCase() === 'ACTIVO' ? 'bg-green-100 text-green-800' :
                                            periferico.estado?.nombre?.toUpperCase() === 'ASIGNADO' ? 'bg-blue-100 text-blue-800' :
                                                periferico.estado?.nombre?.toUpperCase() === 'DAÑADO' ? 'bg-red-100 text-red-800' :
                                                    'bg-gray-100 text-gray-800'
                                            }`}>
                                            {periferico.estado?.nombre || 'Sin Estado'}
                                        </span>
                                    </td>
                                    <td className="px-6 py-4">
                            ...
                            <tr>
                                <td colSpan={7} className="px-6 py-8 text-center text-gray-500">
                                    <div className="flex flex-col items-center justify-center">
                                        ...
                                        <p>No se encontraron periféricos que coincidan con los filtros.</p>
                                    </div>
                                </td>
                            </tr>
                        )}
                ...
                <div className="hidden sm:flex-1 sm:flex sm:items-center sm:justify-between">
                    <div>
                        <p className="text-sm text-gray-700 m-0">
                            Mostrando <span className="font-medium">{currentPerifericos.length}</span> de <span className="font-medium">{filteredPerifericos.length}</span> periféricos
                        </p>
                    </div>
                    <div>
