...
import { createPcEscritorio } from '../../servicios/pcEscritorioService';
...
const FormularioCrearPcEscritorio = ({ handleClose }: { handleClose: () => void }) => {
    ...
    useEffect(() => {
        const cargarDatos = async () => {
            try {
                const [funcionariosData, estadosData] = await Promise.all([
                    getFuncionarios(),
                    getEstados()
                ]);
    ...
    const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
        ...
        if (!formData.marca || !formData.modelo || !formData.serial || !formData.estado.id) {
            Swal.fire({
                icon: 'error',
        ...
        try {
            ...
            await createPcEscritorio(dataToSend);

            Swal.fire({
                icon: 'success',
                title: 'PC Escritorio Creado',
                text: 'El PC de escritorio fue creado correctamente.',
            });
            ...
            console.error("Error al crear el PC Escritorio:", error);
            Swal.fire({
                icon: 'error',
                title: 'Error',
                text: 'No se pudo crear el PC de escritorio.',
            });
        }
    };
...
export default FormularioCrearPcEscritorio;
