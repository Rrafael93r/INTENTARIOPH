...
import { getDiademaById, updateDiadema } from '../../servicios/DiademaService';
...
interface FormularioEditarDiademaProps {
    id: number;
    handleClose: () => void;
    onSuccess: () => void;
}
...
const FormularioEditarDiadema: React.FC<FormularioEditarDiademaProps> = ({ id, handleClose, onSuccess }) => {
    ...
    useEffect(() => {
        const cargarDatos = async () => {
            try {
                const [diademaData, funcionariosData, marcasData] = await Promise.all([
                    getDiademaById(id),
                    getFuncionarios(),
                    getMarcas()
                ]);
                ...
                setFormData({
                    marca: diademaData.marca ? { id: diademaData.marca.id } : { id: '' },
                    modelo: diademaData.modelo || '',
                    serial: diademaData.serial || '',
                    estado: diademaData.estado || '',
                    fecha_compra: diademaData.fecha_compra || '',
                    funcionarios: { id: diademaData.funcionarios?.id || '' },
                    descripcion: diademaData.descripcion || ''
                });
                ...
                Swal.fire({
                    icon: 'error',
                    title: 'Error',
                    text: 'Error al cargar los datos de la diadema',
                });
            }
        };
        if (id) {
    ...
    const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
        ...
        try {
            ...
            await updateDiadema(id, dataToSend);

            Swal.fire({
                icon: 'success',
                title: 'Diadema Actualizada',
                text: 'La diadema fue actualizada correctamente.',
            });
            ...
            console.error("Error al actualizar la diadema:", error);
            Swal.fire({
                icon: 'error',
                title: 'Error',
                text: 'No se pudo actualizar la diadema.',
            });
        }
    };
...
export default FormularioEditarDiadema;
