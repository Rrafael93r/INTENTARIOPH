...
import { getHubUsbById, updateHubUsb } from '../../servicios/hubUsbService';
...
interface FormularioEditarHubUsbProps {
    id: number;
    handleClose: () => void;
    onSuccess: () => void;
}

const FormularioEditarHubUsb: React.FC<FormularioEditarHubUsbProps> = ({ id, handleClose, onSuccess }) => {
    ...
    useEffect(() => {
        const cargarDatos = async () => {
            try {
                const [hubUsbData, funcionariosData] = await Promise.all([
                    getHubUsbById(id),
                    getFuncionarios()
                ]);
                ...
                setFormData({
                    marca: hubUsbData.marca || '',
                    modelo: hubUsbData.modelo || '',
                    serial: hubUsbData.serial || '',
                    estado: hubUsbData.estado || '',
                    fecha_compra: hubUsbData.fecha_compra || '',
                    funcionarios: { id: hubUsbData.funcionarios?.id || '' },
                    descripcion: hubUsbData.descripcion || ''
                });
                ...
                Swal.fire({
                    icon: 'error',
                    title: 'Error',
                    text: 'Error al cargar los datos del hub usb',
                });
            }
        };
        if (id) {
    ...
    const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
        ...
        try {
            ...
            await updateHubUsb(id, dataToSend);

            Swal.fire({
                icon: 'success',
                title: 'Hub USB Actualizado',
                text: 'El hub usb fue actualizado correctamente.',
            });
            ...
            console.error("Error al actualizar el hub usb:", error);
            Swal.fire({
                icon: 'error',
                title: 'Error',
                text: 'No se pudo actualizar el hub usb.',
            });
        }
    };
...
export default FormularioEditarHubUsb;
