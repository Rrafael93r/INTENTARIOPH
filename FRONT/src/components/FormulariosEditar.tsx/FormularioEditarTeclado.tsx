...
import { getTecladoById, updateTeclado } from '../../servicios/tecladosService';
...
interface FormularioEditarTecladoProps {
    id: number;
    handleClose: () => void;
    onSuccess: () => void;
}

const FormularioEditarTeclado: React.FC<FormularioEditarTecladoProps> = ({ id, handleClose, onSuccess }) => {
    ...
    useEffect(() => {
        const cargarDatos = async () => {
            try {
                const [tecladoData, funcionariosData] = await Promise.all([
                    getTecladoById(id),
                    getFuncionarios()
                ]);
                ...
                setFormData({
                    marca: tecladoData.marca || '',
                    modelo: tecladoData.modelo || '',
                    serial: tecladoData.serial || '',
                    estado: tecladoData.estado || '',
                    fecha_compra: tecladoData.fecha_compra || '',
                    funcionarios: { id: tecladoData.funcionarios?.id || '' }
                });
                ...
                Swal.fire({
                    icon: 'error',
                    title: 'Error',
                    text: 'Error al cargar los datos del teclado',
                });
            }
        };
        if (id) {
    ...
    const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
        ...
        try {
            ...
            await updateTeclado(id, dataToSend);

            Swal.fire({
                icon: 'success',
                title: 'Teclado Actualizado',
                text: 'El teclado fue actualizado correctamente.',
            });
            ...
            Swal.fire({
                icon: 'error',
                title: 'Error',
                text: 'No se pudo actualizar el teclado.',
            });
        }
    };
...
export default FormularioEditarTeclado;
